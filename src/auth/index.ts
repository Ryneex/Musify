...
class Auth {
    ...
    async getCurrentUser(option: { withSession: boolean } | undefined = undefined) {
        ...
        try {
            await this.dbconnect();
            const session = await this.SessionModel.findById(session_id.value, { __v: false }).populate("user", { __v: false });
