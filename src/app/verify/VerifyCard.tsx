...
export default function VerifyCard({ email }: any) {
    ...
    return (
        <div className="flex h-screen items-center justify-center sm:bg-slate-50">
            <div
            ...
            <div className="w-full max-w-[640px] rounded-xl bg-white p-10 sm:shadow-md">
                {sendNowClicked ? (
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            >
                                <InputOTPGroup>
                                    <InputOTPSlot className="h-12 w-12 sm:w-16" index={0} />
                                    <InputOTPSlot className="h-12 w-12 sm:w-16" index={1} />
                                    <InputOTPSlot className="h-12 w-12 sm:w-16" index={2} />
                                    <InputOTPSlot className="h-12 w-12 sm:w-16" index={3} />
                                    <InputOTPSlot className="h-12 w-12 sm:w-16" index={4} />
                                    <InputOTPSlot className="h-12 w-12 sm:w-16" index={5} />
                                </InputOTPGroup>
                            </InputOTP>
                            <div className="flex justify-between text-xs">
                                <span className={`${(timer !== 0 || loading) && 'pointer-events-none opacity-70'}`}>
                                    Didn&apos;t receive it?
                                    <span onClick={sendCode} className="cursor-pointer text-blue-600">
                                        {' '}
                                        Resend
                                    </span>
                                </span>
                                {timer !== 0 && <span>Resend in {Duration.fromMillis(timer).toFormat('m:ss')}</span>}
                            </div>
                            <Button
                    ...
                    <div className="flex flex-col items-center gap-5">
                        <h1 className="text-center text-xl font-bold text-black/80">Verify your Email Address</h1>
                        <p className="text-center text-sm">
                            A verification email will be sent to <span className="text-indigo-600">{email}</span>. make
                            sure to check spam mail. Please click on verify now to proceed
                        </p>
                        <Button
                            disabled={loading}
                            loading={loading}
